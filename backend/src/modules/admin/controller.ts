...
export const dashboard = async (_req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const statusBreakdown = statusAgg.reduce((acc, row) => { acc[row._id] = row.count; return acc; }, {});
        const response = {
            status: 'success',
            data: { totals: { users: totalUsers, products: totalProducts, orders: totalOrders, revenue: revenueResult[0]?.totalRevenue || 0 }, statusBreakdown, recentOrders, monthlyRevenue, recentActivity },
        };
...
export const activity = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const [items, total] = await Promise.all([
            AuditLog.find().sort({ createdAt: -1 }).skip((pageNum - 1) * limitNum).limit(limitNum).lean(),
            AuditLog.countDocuments(),
        ]);
        res.json({ status: 'success', pagination: { page: pageNum, limit: limitNum, totalPages: Math.ceil(total / limitNum), total }, data: items });
    } catch (err) { next(err); }
};
...
export const listUsers = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if (search) { const regex = new RegExp(escapeRegex(search), 'i'); filter.$or = [{ name: regex }, { email: regex }, { phone: regex }]; }

        const [users, total] = await Promise.all([
            User.find(filter).select('name email role phone createdAt addresses').sort({ createdAt: -1 }).skip((pageNum - 1) * limitNum).limit(limitNum).lean(),
            User.countDocuments(filter),
        ]);

        const userIds = users.map(u => u._id);
        const orderCounts = await Order.aggregate([{ $match: { user: { $in: userIds } } }, { $group: { _id: '$user', count: { $sum: 1 }, totalSpent: { $sum: '$total' } } }]);
        const orderMap = new Map(orderCounts.map(o => [o._id.toString(), o]));
        const usersWithStats = users.map(u => ({ ...u, orderCount: orderMap.get(u._id.toString())?.count || 0, totalSpent: orderMap.get(u._id.toString())?.totalSpent || 0 }));

        res.json({ status: 'success', pagination: { page: pageNum, totalPages: Math.ceil(total / limitNum), total }, data: usersWithStats });
    } catch (err) { next(err); }
};
...
export const changeUserRole = async (req: Request, res: Response, next: NextFunction) => {
    try {
        const { role } = req.body;
        if (!['customer', 'admin'].includes(role)) return res.status(400).json({ status: 'error', message: 'Role must be customer or admin' });
...
export const lowStock = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const products = await Product.find({ stock: { $lte: threshold }, deletedAt: null }).select('name slug stock type images prices').sort({ stock: 1 }).lean();
...
export const exportOrders = async (req: Request, res: Response, next: NextFunction) => {
    try {
        await logAdminAction({ actor: (req as any).user, action: 'export.orders', entityType: 'order', summary: 'Exported orders CSV', meta: { filters: { status: req.query.status, from: req.query.from, to: req.query.to } } });
        const { status, from, to } = req.query;
        const filter: Record<string, any> = {};
        if (status) filter.status = status;
        if (from || to) { filter.createdAt = {}; if (from) filter.createdAt.$gte = new Date(String(from)); if (to) filter.createdAt.$lte = new Date(String(to)); }
        ...
        const rows = orders.map(o => {
            ...
            return [o.orderNumber, new Date(o.createdAt).toISOString().slice(0, 10), `"${u?.name || 'N/A'}"`, u?.email || '', `"${items}"`, o.subtotal, o.shipping, o.tax, o.total, o.status, o.paymentMethod, o.paymentStatus].join(',');
        }).join('\n');
...
export const exportProducts = async (req: Request, res: Response, next: NextFunction) => {
    try {
        await logAdminAction({ actor: (req as any).user, action: 'export.products', entityType: 'product', summary: 'Exported products CSV' });
        ...
        const rows = products.map(p => [`"${p.name}"`, p.slug, p.type, `"${p.origin}"`, (p.prices as any)?.['50g'] || '', (p.prices as any)?.['100g'] || '', (p.prices as any)?.['200g'] || '', p.stock, p.inStock, p.caffeine, `"${(p.moods || []).join(', ')}"`, p.rating, p.reviewCount, new Date(p.createdAt).toISOString().slice(0, 10)].join(',')).join('\n');
        res.setHeader('Content-Type', 'text/csv');
...
export const exportUsers = async (req: Request, res: Response, next: NextFunction) => {
    try {
        await logAdminAction({ actor: (req as any).user, action: 'export.users', entityType: 'user', summary: 'Exported users CSV (contains PII)' });
        ...
        const rows = users.map(u => [`"${u.name}"`, u.email, u.role, u.phone || '', new Date(u.createdAt).toISOString().slice(0, 10)].join(',')).join('\n');
        res.setHeader('Content-Type', 'text/csv');
...
export const createCoupon = async (req: Request, res: Response, next: NextFunction) => {
    try { res.status(201).json({ status: 'success', data: await Coupon.create(req.body) }); } catch (err) { next(err); }
};
...
export const deleteCoupon = async (req: Request, res: Response, next: NextFunction) => {
    try { await Coupon.findByIdAndDelete(req.params.id); res.status(204).send(); } catch (err) { next(err); }
};
...
export const updateTracking = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const order = await Order.findByIdAndUpdate(req.params.id, { trackingNumber, trackingUrl }, { new: true });
...
export const invoice = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        let y = 125;
        doc.fontSize(11).font('Helvetica-Bold').text('Bill To:', 50, y); y += 16;
        doc.fontSize(10).font('Helvetica').text(`${addr?.firstName || ''} ${addr?.lastName || ''}`, 50, y).text([addr?.line1, addr?.line2, addr?.city, addr?.state, addr?.pincode].filter(Boolean).join(', '), 50, y + 14).text(`Phone: ${addr?.phone || 'N/A'}`, 50, y + 28).text(`Email: ${u?.email || 'N/A'}`, 50, y + 42);

        y = 220;
        doc.moveTo(50, y).lineTo(545, y).stroke('#ddd'); y += 8;
        ...
        y += 16; doc.moveTo(50, y).lineTo(545, y).stroke('#ddd'); y += 8;
        ...
        y += 8; doc.moveTo(350, y).lineTo(545, y).stroke('#ddd'); y += 10;
        doc.text('Subtotal', 380, y).text(`₹${order.subtotal?.toLocaleString('en-IN')}`, 460, y, { width: 85, align: 'right' }); y += 16;
        doc.text('Shipping', 380, y).text(order.shipping === 0 ? 'FREE' : `₹${order.shipping}`, 460, y, { width: 85, align: 'right' }); y += 16;
        doc.text('Tax (GST 5%)', 380, y).text(`₹${order.tax}`, 460, y, { width: 85, align: 'right' }); y += 18;
        doc.moveTo(350, y).lineTo(545, y).stroke('#ddd'); y += 10;
...
export const listTestimonials = async (_req: Request, res: Response, next: NextFunction) => {
    try { res.json({ status: 'success', data: await Testimonial.find().sort({ order: 1, createdAt: -1 }).lean() }); } catch (err) { next(err); }
};
...
export const createTestimonial = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if (!author || !text) return res.status(400).json({ status: 'error', message: 'Author and text are required' });
        const testimonial = await Testimonial.create({ author: author.trim(), role: role?.trim() || 'Customer', text: text.trim(), rating: rating || 5, approved: approved !== undefined ? approved : false, featured: featured || false, order: order || 0 });
