...
document.addEventListener('DOMContentLoaded', () => {
  ...
  authModal.innerHTML = `
    ...
    <div class="auth-modal__dialog">
      ...
      <div class="auth-modal__header">
        ...
        <p class="auth-modal__subtitle">Sign in to your Serené account</p>
      </div>
  ...
  function switchAuthTab(tab) {
    ...
    if (tab === 'login') {
      ...
      authSubtitle.textContent = 'Sign in to your Serené account';
      ...
      authSubtitle.textContent = 'Join the Serené Tea community';
  ...
  signupForm.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      saveAuthData(data.data);
      closeAuthModal();
      showToast(`Welcome to Serené, ${data.data.user.name}!`, 'success');
      updateHeaderAuth();
    } catch (err) {
      showAuthError(err.message);
    } finally {
  ...
  function saveAuthData(data) {
    localStorage.setItem('serene_token', data.accessToken);
    localStorage.setItem('serene_refresh', data.refreshToken);
    localStorage.setItem('serene_user', JSON.stringify(data.user));
  }

  function getUser() {
    try { return JSON.parse(localStorage.getItem('serene_user')); } catch { return null; }
  }
  ...
  function logout() {
    ...
    localStorage.removeItem('serene_refresh');
  ...
  function updateHeaderAuth() {
    ...
    if (user) {
      ...
      accountLink.href = '#';
      accountLink.setAttribute('aria-label', `${user.name} — Click to sign out`);
