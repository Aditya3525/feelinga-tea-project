...
export default function SearchOverlay({ isOpen, onClose }) {
    ...
    const handleSearch = async (e) => {
        ...
        try {
            const res = await fetch(`/api/v1/products?search=${encodeURIComponent(val)}&limit=5`);
        ...
        <div className="search-overlay active" role="dialog" aria-label="Search">
            <div className="search-overlay__inner">
                ...
                <div className="search-overlay__tags">
                    {tags.map(tag => (
                        <button key={tag} className="search-tag" onClick={() => { router.push(`/shop?search=${encodeURIComponent(tag)}`); onClose(); }}>{tag}</button>
                    ))}
                </div>
                {results.length > 0 && (
                    <div className="search-overlay__results">
                        {results.map(product => (
                            <button key={product._id} className="search-result" onClick={() => { router.push('/product'); onClose(); }}>
