...
router.get('/', async (req, res, next) => {
    try {
        ...
        const { type, mood, caffeine, minPrice, maxPrice, sort = '-createdAt', page = 1, limit = 12, q, isNewArrival, isBestSeller } = req.query;
        const filter = {};
        if (type)
            filter.type = type;
        if (mood)
            filter.moods = { $in: Array.isArray(mood) ? mood : [mood] };
        if (caffeine)
            filter.caffeine = caffeine;
        if (minPrice || maxPrice) {
            filter['prices.100g'] = {};
            if (minPrice)
                filter['prices.100g'].$gte = Number(minPrice);
            if (maxPrice)
                filter['prices.100g'].$lte = Number(maxPrice);
        }
...
router.get('/search', async (req, res, next) => {
    try {
        ...
        const textResults = await Product.find({ $text: { $search: term } }, { score: { $meta: 'textScore' } })
...
router.get('/autocomplete', async (req, res, next) => {
    try {
        ...
        const suggestions = await Product.find({ name: regex }, { name: 1, slug: 1, type: 1, 'prices.100g': 1, images: { $slice: 1 } }).limit(6).lean();
        res.json({
            status: 'success',
            data: suggestions,
        });
...
router.patch('/bulk-stock', authenticate, authorize('admin'), validate(bulkStockSchema), async (req, res, next) => {
    try {
        ...
        const result = await Product.updateMany({ _id: { $in: productIds } }, { $set: { stock, inStock: stock > 0 } });
        cache.invalidate('products:');
        await logAdminAction({
            actor: req.user,
