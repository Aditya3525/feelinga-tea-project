        ...
        <div className="auth-modal active">
            ...
            <div className="auth-modal__dialog">
                ...
                <div className="auth-modal__header">
                    <h2 className="auth-modal__title">{activeTab === 'login' ? 'Welcome Back' : 'Create Account'}</h2>
                    <p className="auth-modal__subtitle">{activeTab === 'login' ? 'Sign in to your feelinga account' : 'Join the tea community'}</p>
                </div>
                <div className="auth-modal__tabs" role="tablist">
                    <button className={`auth-modal__tab ${activeTab === 'login' ? 'active' : ''}`} onClick={() => { setActiveTab('login'); setError(''); }}>Login</button>
                    <button className={`auth-modal__tab ${activeTab === 'signup' ? 'active' : ''}`} onClick={() => { setActiveTab('signup'); setError(''); }}>Sign Up</button>
                </div>
                ...
                {activeTab === 'login' ? (
                    <form className="auth-modal__form" onSubmit={handleLogin}>
                        ...
                        <button type="submit" className="btn btn--primary auth-modal__submit" disabled={loading}>{loading ? 'Signing in...' : 'SIGN IN'}</button>
                    </form>
                ) : (
                    <form className="auth-modal__form" onSubmit={handleRegister}>
                        ...
                        <button type="submit" className="btn btn--primary auth-modal__submit" disabled={loading}>{loading ? 'Creating...' : 'CREATE ACCOUNT'}</button>
                    </form>
                )}

                <div className="auth-modal__divider"><span>OR</span></div>
                <button className="auth-modal__google" onClick={() => showToast('Google login requires backend configuration', 'info')}>
                ...
                <div className="auth-modal__footer">
                    {activeTab === 'login' ? (
                        <span>Don&apos;t have an account? <button className="auth-modal__toggle" onClick={() => { setActiveTab('signup'); setError(''); }}>Sign Up</button></span>
                    ) : (
                        <span>Already have an account? <button className="auth-modal__toggle" onClick={() => { setActiveTab('login'); setError(''); }}>Login</button></span>
                    )}
                </div>
            </div>
        </div>
